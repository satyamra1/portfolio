...
const sV = { hidden: { opacity: 0, y: 30 }, show: { opacity: 1, y: 0, transition: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } } }

function AnimSection({ children, delay = 0, direction = 'up' }) {
  const [ref, inView] = useInView({ threshold: 0.1, triggerOnce: true })
  const variants = {
    hidden: { opacity: 0, x: direction === 'left' ? -40 : direction === 'right' ? 40 : 0, y: direction === 'up' ? 30 : 0 },
    show: { opacity: 1, x: 0, y: 0, transition: { duration: 0.75, delay, ease: [0.22, 1, 0.36, 1] } }
  }
  return <motion.div ref={ref} variants={variants} initial="hidden" animate={inView ? 'show' : 'hidden'}>{children}</motion.div>
}
  ...
  return (
    <section id="about" style={{ background: 'var(--bg-2)', padding: '6rem 0' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '5rem', alignItems: 'center',
          ...
          <AnimSection direction="right">
            ...
            <p style={{ color: 'var(--muted)', lineHeight: 1.8, marginBottom: '2rem', fontSize: '1rem' }}>
              My expertise spans frontend engineering, backend architecture, databases, cloud deployment, payment integrations, authentication systems, and AI-driven workflows. I enjoy solving meaningful business problems through technology while continuously learning and adapting.
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
              {[
                ...
                <div key={s.label} style={{
                  ...
                  borderRadius: 12, padding: '1.1rem', textAlign: 'center', backdropFilter: 'blur(12px)',
                }}>
